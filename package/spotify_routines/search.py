...
def _store_not_found_queries(queries: list[str]) -> None:
    ...
    with open(path.join(SPOTIFY_SEARCH_OUTPUT_DIR, SPOTIFY_NOT_FOUND_FILE), 'a') as oFile:
...
def _search(spotify: sp.Spotify, input_tidal_file, query_builder, spotify_search_type, result_validator, id_accessor) -> list[str]:
    ...
    for query in queries:
        result = spotify.search(query, type=spotify_search_type)
...
def search_artists_ids(spotify: sp.Spotify) -> list[str]:
    ...
    return _search(spotify, 
                  ...
                  query_builder=lambda e:e[kArtist],
                  spotify_search_type=kArtist,
                  result_validator=lambda r:len(r[kArtists][kItems]) > 0,
                  id_accessor=lambda r:r[kArtists][kItems][0][kId])
...
def search_albums_ids(spotify: sp.Spotify) -> list[str]:
    ...
    return _search(spotify, 
                  ...
                  query_builder=lambda e:f'artist:{e[kArtist]} album:{e[kName]}',
                  spotify_search_type=kAlbum,
                  result_validator=lambda r:len(r[kAlbums][kItems]) > 0,
                  id_accessor=lambda r:r[kAlbums][kItems][0][kId])
...
def search_tracks_ids(spotify: sp.Spotify) -> list[str]:
    ...
    return _search(spotify, 
                  ...
                  query_builder=lambda e:f'artist:{e[kArtist]} track:{e[kName]}',
                  spotify_search_type=kTrack,
                  result_validator=lambda r:len(r[kTracks][kItems]) > 0,
                  id_accessor=lambda r:r[kTracks][kItems][0][kId])